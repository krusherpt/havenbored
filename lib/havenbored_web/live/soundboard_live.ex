defmodule HavenboredWeb.SoundboardLive do
  use HavenboredWeb, :live_view
  use HavenboredWeb.Live.Support.PresenceLive
  alias HavenboredWeb.Components.Havenbored.{DeleteModal, EditModal, UploadModal}
  import EditModal
  import DeleteModal
  import UploadModal
  import HavenboredWeb.Components.Havenbored.TagComponents, only: [tag_filter_button: 1]
  alias Havenbored.{Favorites, PubSubTopics, Sounds}
  alias HavenboredWeb.Live.SoundboardLive.{EditFlow, UploadFlow}
  alias HavenboredWeb.Live.Support.{FlashHelpers, SoundPlayback}
  alias HavenboredWeb.Havenbored.SoundFilter
  import HavenboredWeb.Live.Support.LiveTags, only: [all_tags: 1, tag_selected?: 2]
  ...
  def handle_event("stop_sound", _params, socket) do
    ...
    if socket.assigns.current_user do
      Havenbored.AudioPlayer.stop_sound()
    end
