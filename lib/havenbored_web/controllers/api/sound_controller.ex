defmodule HavenboredWeb.API.SoundController do
  use HavenboredWeb, :controller

  alias Havenbored.{Repo, Sound, Sounds}
    ...
    case Repo.get(Sound, id) do
      ...
      sound ->
        case require_play_user(conn) do
          {:ok, user} ->
            Havenbored.AudioPlayer.play_sound(sound.filename, user)
  ...
  defp require_upload_user(conn) do
    case conn.assigns[:current_user] do
      %Havenbored.Accounts.User{} = user -> {:ok, user}
