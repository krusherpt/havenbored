defmodule Havenbored.Sounds.Management do
  @moduledoc """
  ...
  alias Havenbored.{AudioPlayer, Repo, Sound, UploadsPath, Volume}
  require Logger
  ...
  defp update_user_settings(sound, user_id, updated_sound, params) do
    user_setting =
      Enum.find(sound.user_sound_settings, &(&1.user_id == user_id)) ||
        %Havenbored.UserSoundSetting{sound_id: sound.id, user_id: user_id}
    ...
    Havenbored.UserSoundSetting.clear_conflicting_settings(
      user_id,
      sound.id,
    ...
    case user_setting
         |> Havenbored.UserSoundSetting.changeset(setting_params)
