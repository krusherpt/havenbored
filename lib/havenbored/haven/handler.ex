defmodule Havenbored.Haven.Handler do
  @moduledoc """
  ...
  alias Havenbored.Haven.{Channel, WebhookClient}
  ...
  def handle_call({:play_sound, sound_name, _actor}, _from, %{channel: channel}) do
    case WebhookClient.play_sound(channel.webhook_token, sound_name) do
      :ok ->
        Havenbored.AudioPlayer.Notifier.sound_played(sound_name, "Haven")
        {:reply, :ok, state}

      {:error, reason} ->
        Logger.error("Failed to play sound via Haven: #{inspect(reason)}")
        Havenbored.AudioPlayer.Notifier.error("Failed to play sound: #{inspect(reason)}")
  ...
  defp play_sound_from_command(sound_name, _actor) do
    case Process.whereis(Havenbored.Haven.WebhookClient) do
      nil ->
        Logger.warning("WebhookClient not running")

      _pid ->
        # Use the channel's webhook token to play the sound
        channel = Havenbored.Haven.Handler.get_channel()
