defmodule Havenbored.Haven.HandlerTest do
  ...
  describe "dispatch_event/1" do
    test "accepts message events" do
      assert :ok ==
               Havenbored.Haven.Handler.dispatch_event(%{
                 "event" => "message",
                 "message" => %{
                   "content" => "Hello world",
    ...
    test "returns error when handler is not running" do
      ...
      Process.exit(Process.whereis(Havenbored.Haven.Handler), :kill)
      :timer.sleep(100)
  ...
  describe "get_channel/0" do
    test "returns the configured channel" do
      assert {:ok, channel} = Havenbored.Haven.Handler.get_channel()
  ...
  describe "play_sound/2" do
    test "returns :error when webhook is not configured" do
      ...
      Havenbored.Haven.Handler.stop()
      :timer.sleep(100)
      {:ok, _pid} = GenServer.start_link(Havenbored.Haven.Handler, [])

      assert {:error, :not_configured} == Havenbored.Haven.Handler.play_sound("Test", %{})
    end
  end
end
