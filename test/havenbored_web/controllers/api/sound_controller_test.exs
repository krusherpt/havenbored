defmodule HavenboredWeb.API.SoundControllerTest do
  @moduledoc """
  ...
  alias Havenbored.{Repo, Sound, Tag, UserSoundSetting}
  ...
  describe "play" do
    test "plays a sound as the authenticated token user", %{conn: conn, sound: sound, user: user} do
      with_mock Havenbored.AudioPlayer, play_sound: fn _filename, _actor -> :ok end do
        ...
        assert_called(Havenbored.AudioPlayer.play_sound(sound.filename, user))
      end
    end
    ...
    test "returns error when sound not found", %{conn: conn} do
      with_mock Havenbored.AudioPlayer, play_sound: fn _filename, _username -> :ok end do
